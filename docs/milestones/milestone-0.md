...
- `src/ledger/schema.d.ts` — **generated**, 8798 lines / all 56 JSON Ledger API v2 endpoints, via `npm run gen:ledger-client` → `openapi-typescript http://localhost:7575/docs/openapi`. See "Issues hit" #4 for why that route (not `/v2/openapi.json`) is the correct one.
...
- `git init` done, default branch renamed to `main`, initial commit made (`46a900c`).
...
4. **`GET /v2/openapi.json` (and every other `/v2/...` docs/swagger path tried) 404s on `dpm sandbox`** — resolved. The correct route is **`GET /docs/openapi`** (note: no `/v2` prefix, no `.json` suffix, `text/plain` content-type but the body is a YAML-encoded OpenAPI 3.0.3 document — `openapi-typescript` handles this fine given a URL). Found by disassembling `canton-open-source-3.5.6.jar` with `javap`: `com.digitalasset.canton.http.json.v2.JsApiDocsService` builds its endpoint via `baseEndpoint / "docs" / "openapi"`, where `baseEndpoint` does **not** carry the `/v2` prefix (that prefix is added per-route elsewhere, not on the docs route). Along the way, a `damlDefinitionsServiceEnabled` config flag (`canton.participants.<name>.http-ledger-api.daml-definitions-service-enabled`, default `false`) looked like a promising suspect and was tested via a `-c` config overlay — but the route works identically with or without it, so that flag gates something else and was a dead end, not the fix. `backend/src/ledger/schema.d.ts` is now genuinely generated (8798 lines, all 56 endpoints) via `npm run gen:ledger-client`.
