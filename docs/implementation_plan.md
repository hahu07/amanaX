...
- Party IDs (`hint::namespace_fingerprint`) can only be minted by a running participant node — not generated client-side. Allocate via `POST /v2/parties/allocate` (the old `daml ledger allocate-parties` is deprecated).
